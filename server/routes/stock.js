...
router.get('/search', (req, res) => {
    let sql = "SELECT * FROM stock WHERE NAME LIKE \'%"+req.query.name+"%\'";
    model.querySQL(sql).then(ret => {
        res.send({status: 0, data: ret})
    }, err => {
        res.send({status: 1, msg: err})
...
//根据日志的stockid搜索类型为修改的库存日志
router.get('/stocklog-search', (req, res) => {
    let sql = "SELECT * FROM stocklog WHERE stockid="+req.query.stockid+" AND operation=\'"+req.query.operation+"\'";
    console.log(sql);
    model.querySQL(sql).then(ret => {
        res.send({status: 0, data: ret})
    }, err => {
